...
export default function CartPage() {
  ...
  const [couponInput, setCouponInput] = useReactState("");
  const [savedForLater, setSavedForLater] = useReactState<string[]>([]);
  const [recommendations, setRecommendations] = useReactState<any[]>([]);

  useEffect(() => {
    import("@/lib/actions/product").then(({ getShopProducts }) => {
      getShopProducts().then((products) => {
        // Just slice the first 6 products as recommendations for now
    ...
    return (
      <div className="container-app flex flex-col items-center justify-center gap-4 py-24 text-center">
        ...
        <p className="max-w-sm text-sm text-ink/50">
          Browse our decoration catalog and add a package to get started with
          your booking.
        </p>
        ...
        <div className="space-y-4">
          {items.map((item) => {
            ...
            const addOnNames = (item.addOnIds || [])
              .map(
                (id) =>
                  product.addOns?.find((pa: any) => pa.addOnId === id)?.addOn
                    ?.name,
              )
              .filter(Boolean);
            ...
            return (
              <div
                key={item.cartId}
                className="flex flex-col gap-4 rounded-2xl border border-ink/8 bg-white p-4 sm:flex-row"
              >
                  ...
                  <div className="flex items-start justify-between gap-2">
                    <div>
                      <Link
                        ...
                        className="font-semibold hover:text-grape-700"
                      >
                        {product.name}
                      </Link>
                      <p className="text-xs text-ink/50">
                        {item.city} · {item.venue} · {item.eventDate} at{" "}
                        {item.eventTime}
                      </p>
                    </div>
                    <span className="font-mono text-base font-bold">
                      {formatPrice(
                        (product.price + addOnsTotal) * item.quantity,
                      )}
                    </span>
                  </div>
            ...
            {couponCode ? (
              <div className="flex items-center justify-between rounded-xl bg-leaf-50 px-3 py-2 text-sm">
                ...
                <button
                  ...
                  className="text-xs font-semibold text-ink/50 hover:text-punch-500"
                >
                  Remove
                </button>
              </div>
