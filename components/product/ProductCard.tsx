...
export default function ProductCard({
  ...
  const cityAvailability =
    (product as any).cityAvailabilities?.find(
      (c: any) =>
        c.city?.slug === city ||
        c.city?.name?.toLowerCase() === city?.toLowerCase(),
    ) || product.cities?.find((c) => c.city === city);
  ...
  return (
    ...
    >
      ...
      >
        <Image
          ...
          className={cn(
            ...
            hover && product.images?.[1] && "opacity-0",
          )}
        />
        {product.images?.[1] && (
          <Image
        ...
        <div className="absolute left-2 top-2 flex flex-col gap-1">
          {discount > 0 && (
            <span className="badge bg-punch-500 text-white">
              {discount}% OFF
            </span>
          )}
        ...
        >
          <Heart
            size={16}
            className={
              wishlisted ? "fill-punch-500 text-punch-500" : "text-ink/50"
            }
          />
        </button>
      ...
      <div className="flex flex-1 flex-col gap-1.5 p-3">
        <Link
          href={`/product/${product.slug}`}
          className="line-clamp-1 text-sm font-semibold text-ink hover:text-grape-700"
        >
          {product.name}
        </Link>
        ...
        <div className="flex items-center gap-1.5 text-xs">
          {product.reviews && product.reviews.length > 0 ? (
            <>
              ...
              <span className="text-ink/40">({product.reviews.length})</span>
            </>
          ) : (
            <span className="flex items-center gap-0.5 rounded bg-ink/5 px-1.5 py-0.5 font-medium text-ink/50">
              New
            </span>
          )}
        </div>
