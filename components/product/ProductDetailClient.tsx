...
function getBalloonColors(choice: string): string[] {
  ...
  if (choice.startsWith("Custom: ")) {
    ...
    return names.map(
      (n) => SINGLE_BALLOON_COLORS.find((c) => c.name === n)?.hex || "#000",
    );
  }
...
export default function ProductDetailClient({ product }: { product: Product }) {
  ...
  const DATE_STRIP = useMemo(
    () => (isMounted ? getDateStrip() : []),
    [isMounted],
  );
  ...
  const customizationSummary = Object.values(customizations)
    .filter(Boolean)
    .join(", ");
  ...
  return (
    <div className="container-app py-5">
      ...
      <nav className="mb-5 flex items-center gap-1.5 text-xs text-ink/45">
        <Link href="/" className="hover:text-grape-700 transition-colors">
          Home
        </Link>
        <ChevronRight size={11} className="opacity-50" />
        <Link
          href={`/decorations/${categorySlug}`}
        ...
        <div className="flex flex-col gap-5 lg:col-span-7">
            ...
            <div className="flex items-start justify-between gap-3">
              <div className="min-w-0">
                <div className="mb-2 flex flex-wrap gap-1.5">
                  <span className="badge bg-grape-100 text-grape-700 uppercase tracking-widest text-[9px] font-bold px-2 py-0.5">
                    {product.category}
                  </span>
                </div>
                ...
                <p className="mt-2 text-[15px] text-ink/60">
                  {product.tagline ||
                    "Celebrate your years together with a photo memory wall"}
                </p>
                {/* Rating below title */}
                <a
                  href="#tabs"
                  className="mt-3 flex w-fit items-center gap-1.5 text-sm"
                >
                  {product.reviews && product.reviews.length > 0 ? (
                    <>
                      <Star
                        size={16}
                        className="fill-grape-700 text-grape-700"
                      />
                      <span className="font-bold text-grape-900">
                        {(
                      ...
                      <span className="text-ink/50 hover:underline">
                        {product.reviews.length} reviews
                      </span>
                    </>
                  ) : (
                    <span className="rounded bg-ink/5 px-2 py-0.5 text-xs font-medium text-ink/50">
                      New
                    </span>
                  )}
                </a>
          ...
          <div className="rounded-3xl bg-white border border-ink/5 shadow-[0_4px_20px_rgb(0,0,0,0.04)] p-6">
            <div className="flex items-center gap-3">
              <span className="font-display text-4xl font-bold text-grape-900">
                {formatPrice(product.price)}
              </span>
              {product.mrp > product.price && (
                <>
                  <span className="text-lg font-medium text-ink/40 line-through">
            ...
            {product.mrp > product.price && (
              <div className="mt-3 inline-flex items-center gap-1.5 rounded-full bg-leaf-50 px-3 py-1.5 text-xs font-bold text-leaf-700">
                ...
                You save {formatPrice(product.mrp - product.price)} on this
                package
              </div>
            )}
            ...
            <div className="mt-4 flex items-center gap-2 rounded-xl bg-marigold-50/50 px-4 py-3 text-[13px] font-semibold text-marigold-800 border border-marigold-100/50">
              ...
              {cityAvailability?.status === "available" &&
                `Available in ${city} - Earliest slot ${cityAvailability.earliestSlot}`}
              {cityAvailability?.status === "limited" &&
                `Limited slots in ${city} - ${cityAvailability.earliestSlot}`}
              {(!cityAvailability ||
                cityAvailability.status === "unavailable") &&
                `Not available in ${city} yet — contact us`}
            </div>
          ...
          <div className="relative overflow-hidden rounded-3xl bg-white border border-ink/5 shadow-[0_4px_20px_rgb(0,0,0,0.04)] p-6 z-10">
            ...
            <ul className="grid grid-cols-1 gap-y-4 sm:grid-cols-2 gap-x-6">
              {product.whatsIncluded.map((w) => (
                <li
                  key={w}
                  className="flex items-start gap-3 text-[14px] font-medium text-ink/70 leading-relaxed"
                >
            ...
            <div className="rounded-3xl bg-white border border-ink/5 shadow-[0_4px_20px_rgb(0,0,0,0.04)] p-6 z-10">
              ...
              <div className="space-y-5">
                {product.customizations.map((c) => (
                  <div key={c.id}>
                    <label className="mb-2.5 block text-xs font-bold uppercase tracking-wider text-ink/40">
                      {c.label}
                    </label>
                      ...
                      <div className="flex flex-col gap-2">
                        <div className="flex flex-wrap gap-2.5">
                              ...
                              >
                                ...
                                <span
                                  className={cn(
                                    ...
                                    isActive ? "text-grape-600" : "text-ink/60",
                                  )}
                                >
                                  {choice}
                                </span>
                                {isActive && (
                                  <CheckCircle2
                                    size={16}
                                    className="absolute -right-2 -top-2 text-white fill-grape-600 drop-shadow-sm"
                                  />
                                )}
                              </button>
                            );
                          ...
                          <button
                            ...
                            onClick={() =>
                              setShowColorPickerFor(
                                showColorPickerFor === c.id ? null : c.id,
                              )
                            }
                            className={cn(
                          ...
                          >
                            {customizations[c.id]?.startsWith("Custom: ") ? (
                              <div className="flex -space-x-1.5 pt-0.5">
                                {getBalloonColors(customizations[c.id]).map(
                                  (color, i) => (
                                    <BalloonIcon
                                      key={i}
                                      color={color}
          ...
          <div className="relative rounded-3xl border border-ink/5 bg-white shadow-[0_4px_20px_rgb(0,0,0,0.04)] p-6 z-10">
            ...
            <div className="flex gap-2.5 overflow-x-auto scrollbar-none pb-2 pt-4 -mt-2 -mx-2 px-2 snap-x snap-mandatory">
              {!isMounted ? (
                Array.from({ length: 6 }).map((_, i) => (
                  <div
                    key={i}
                    className="h-[92px] w-[72px] shrink-0 rounded-[14px] border border-ink/5 bg-paper/50 animate-pulse snap-start"
                  />
                ))
              ) : (
                <>
                    ...
                    >
                      {d.isFast && (
                        <span
                          className={cn(
                            ...
                            eventDate === d.iso
                              ? "bg-white text-punch-600"
                              : "bg-gradient-to-r from-punch-500 to-punch-600 text-white",
                          )}
                        >
                          <Zap
                            size={9}
                      ...
                      <span
                        className={cn(
                          ...
                          eventDate === d.iso ? "opacity-90" : "text-ink/40",
                        )}
                      >
                        {d.dayLabel}
                      </span>
                      <span
                        className={cn(
                          "text-[26px] font-extrabold leading-none my-1",
                          eventDate === d.iso ? "text-white" : "text-ink/90",
                        )}
                      >
                        {d.dateNum}
                      </span>
            ...
            <div className="mt-5 mb-3 flex items-center justify-between">
              ...
              <p className="flex items-center gap-1.5 text-[11px] font-semibold text-leaf-700 bg-leaf-50 px-2 py-0.5 rounded-full border border-leaf-100">
                <Clock size={11} strokeWidth={2.5} /> 2-hr window
              </p>
            </div>
            ...
            <div className="rounded-3xl border border-ink/5 bg-white shadow-[0_4px_20px_rgb(0,0,0,0.04)] p-6 z-10">
              <div className="mb-3 flex items-center justify-between">
                <p className="text-sm font-semibold text-ink">
                  Enhance Your Setup{" "}
                  <span className="font-medium text-ink/40">(optional)</span>
                </p>
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="flex flex-1 flex-col p-2.5">
                          <p className="mb-2 line-clamp-2 text-[12px] font-bold leading-snug text-ink">
                            {a.name}
                          </p>
          ...
          <div className="relative mt-8 overflow-hidden rounded-[24px] bg-white shadow-[0_8px_30px_rgb(0,0,0,0.08)] border border-ink/5">
            ...
            <div className="p-5 sm:p-6 lg:p-7 flex flex-col lg:flex-row gap-6 lg:gap-10 items-start lg:items-center relative z-10">
              ...
              <div className="w-full flex-1">
                ...
                <p className="text-[10px] font-bold tracking-widest text-grape-400 mb-2 uppercase">
                  Your Celebration, Our Care
                </p>
                <p className="text-[17px] font-bold text-ink">Total Amount</p>
                <p className="text-[13px] text-ink/50 mt-0.5 mb-3">
                  Inclusive of all taxes
                </p>

                <div className="flex items-center gap-3 mb-4">
                  <span className="font-display text-[46px] font-extrabold leading-none tracking-tight text-ink">
                    {formatPrice(total)}
                  </span>
                  {product.mrp > product.price && (
                    <div className="flex items-center gap-2 mt-2">
                      <span className="font-mono text-[15px] font-bold text-ink/30 line-through">
                        {formatPrice(product.mrp + addOnsTotal)}
                      </span>
                    ...
                    <div className="flex items-center justify-between w-full relative z-10">
                      <div className="flex items-center gap-4">
                        ...
                        <div className="text-left leading-tight py-1">
                          ...
                          <p className="text-[12px] font-medium text-ink/40 mt-0.5">
                      ...
                      <ChevronRight
                        size={18}
                        className="text-grape-700 font-bold"
                      />
                    </div>
                  }
                />
              ...
              <div className="flex flex-wrap justify-between lg:flex-nowrap items-center gap-5 w-full relative z-10">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div className="leading-tight pl-2">
                    <p className="text-[13px] font-medium text-grape-500">
                      Happy
                    </p>
                    <p className="text-[13px] font-medium text-grape-500">
                      Customers
                ...
                <div className="hidden lg:flex items-center gap-4">
                  <div className="text-grape-400">
                    ...
                    >
                      <path
                        d="M12 2C12 2 15 6 15 11C15 16 12 22 12 22C12 22 9 16 9 11C9 6 12 2 12 2Z"
                        stroke="currentColor"
                        strokeWidth="1.5"
                        strokeLinecap="round"
                  ...
                  <div className="text-right">
                    <p className="text-[9px] font-bold tracking-[0.15em] text-grape-400 uppercase leading-relaxed">
                      Your Moments,
                    </p>
                    <p className="text-[9px] font-bold tracking-[0.15em] text-grape-400 uppercase leading-relaxed">
                      Our Commitment
      ...
      <div className="mt-10 grid grid-cols-1 gap-4 sm:grid-cols-2">
        <div className="rounded-2xl border border-ink/8 p-4">
          ...
          <p className="mt-2 text-xs text-ink/40">
            Est. setup time: {product.setupDurationMins} minutes
          </p>
        </div>
        <div className="rounded-2xl border border-ink/8 p-4">
          <p className="mb-1.5 flex items-center gap-2 text-sm font-semibold">
            <RefreshCcw size={14} className="text-grape-600" /> Cancellation &
            Rescheduling
          </p>
        ...
        <div className="flex border-b border-ink/10">
          {(["reviews", "faqs", "similar"] as const).map((tab) => {
            const LABELS: Record<TabKey, string> = {
              reviews: `Reviews (${computedReviewCount})`,
              faqs: "FAQs",
        ...
        {activeTab === "reviews" && (
          <div className="mt-6 space-y-4" id="reviews">
            ...
            {computedReviewCount > 0 ? (
              <>
                    ...
                    <div className="mt-1.5 flex justify-center gap-0.5">
                      {Array.from({ length: 5 }).map((_, i) => (
                        <Star
                          ...
                          size={14}
                          className={
                            i < Math.round(Number(computedRating))
                  ...
                  <div className="flex-1 space-y-1.5">
                    {[5, 4, 3, 2, 1].map((star) => {
                      ...
                      return (
                        <div
                          key={star}
                          className="flex items-center gap-2 text-[11px] font-bold text-ink/60"
                        >
                          <span className="w-3 text-right">{star}</span>
                          <Star
                            size={10}
                            className="fill-marigold-400 text-marigold-400 shrink-0"
                          />
                          <div className="flex-1 rounded-full bg-ink/5 h-2 overflow-hidden shadow-inner">
                            <div
                              className="h-full rounded-full bg-marigold-400 transition-all duration-500 ease-out"
                              style={{ width: `${pct}%` }}
                            />
                          </div>
                          <span className="w-4 font-semibold text-ink/40">
                            {count}
                          </span>
                        </div>
                      );
                ...
                <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                  {product.reviews.map((r) => (
                    <div
                      key={r.id}
                      className="rounded-2xl border border-ink/8 p-5 bg-white shadow-sm"
                    >
                      <div className="flex items-start justify-between gap-3">
                        <div className="flex items-center gap-3">
                        ...
                        <span className="flex shrink-0 items-center gap-1 rounded-full bg-leaf-50 border border-leaf-100 px-2 py-1 text-[11px] font-bold text-leaf-700">
                          {r.rating}{" "}
                          <Star size={10} className="fill-leaf-600" />
                        </span>
                      </div>
              ...
              <div className="flex flex-col items-center justify-center rounded-2xl border border-dashed border-ink/15 py-12 text-center">
                <Star size={32} className="text-ink/20 mb-3" />
                <p className="font-semibold text-ink">No reviews yet</p>
                <p className="mt-1 text-sm text-ink/50 max-w-[250px]">
