            ...
            {openSlug && (
              ...
              >
                <div className="flex gap-12 rounded-2xl border border-ink/10 bg-white p-8 shadow-xl">
                  {(() => {
                    const cat = navigationConfig.find((c) => c.slug === openSlug)!;
                    return (
                      <>
                        {cat.image && (
                          <div className="w-[240px] shrink-0">
                            <div className="relative mb-3 aspect-[4/3] w-full overflow-hidden rounded-2xl">
                              <Image src={cat.image} alt={cat.label} fill className="object-cover" sizes="240px" />
                            </div>
                            <p className="font-display text-lg font-semibold">{cat.label}</p>
                            {cat.description && <p className="text-sm text-ink/55">{cat.description}</p>}
                          </div>
                        )}
                        
                        {cat.columns.map((colGroups, colIdx) => {
                          const hasLinks = colGroups.some(g => g.links.length > 0);
                          if (!hasLinks) return null;
                          return (
                            <div key={colIdx} className="flex flex-col gap-6 min-w-[140px]">
