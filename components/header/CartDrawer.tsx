            ...
            {items.length === 0 ? (
              <div className="flex flex-1 flex-col items-center justify-center gap-3 px-8 text-center">
                ...
                <p className="text-sm text-ink/45">
                  Add a decoration package to get started.
                </p>
                <Link
                  href="/decorations"
                  onClick={closeDrawer}
              ...
              <>
                <div className="flex-1 space-y-4 overflow-y-auto px-5 py-4">
                  {items.map((item) => {
                    ...
                    return (
                      <div
                        key={item.cartId}
                        className="flex gap-3 rounded-2xl border border-ink/8 p-3"
                      >
                        ...
                        <div className="min-w-0 flex-1">
                          ...
                          <p className="mt-0.5 text-xs text-ink/50 leading-relaxed">
                            {item.city} {item.venue && `· ${item.venue}`}
                            {item.eventDate && ` · ${new Date(item.eventDate).toLocaleDateString('en-IN', { month: 'short', day: 'numeric' })}`}
                ...
                <div className="border-t border-ink/10 px-5 py-4">
                  ...
                  <div className="mb-3 flex justify-between text-base font-bold">
                    <span>Total</span>
                    <span className="font-mono">
                      {formatPrice(Math.max(subtotal - discount, 0))}
                    </span>
                  </div>
                  <Link
                    href="/cart"
